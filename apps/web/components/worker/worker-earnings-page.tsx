    ...
    <div>
          ...
          <div className="grid grid-cols-4 items-end gap-4">
            {data.chart.map((item, index) => (
              <div key={item.label} className="flex flex-col items-center gap-3">
                <div className={index % 2 === 0 ? "dashboard-card-soft flex h-60 w-full items-end p-3" : "dashboard-card-soft flex h-60 w-full items-end bg-accent-50 p-3"}>
                  <div
                ...
                <div className="text-center">
                  <p className="text-sm font-semibold text-dark-800">{item.label}</p>
                  <p className="mt-1 text-xs text-dark-500">{formatCurrency(locale, item.amount)}</p>
                </div>
              </div>
            ))}
          </div>
      ...
      <div className="mt-6 grid gap-6 xl:grid-cols-[1.05fr_0.95fr]">
        <DashboardBlock title={isArabic ? "latest transactions" : "Latest transactions"} eyebrow={isArabic ? "money stream" : "money stream"}>
          <div className="grid gap-4">
            {data.transactions.map((item, index) => (
              <div key={item.id} className={index % 2 === 0 ? "dashboard-card-soft flex flex-wrap items-center justify-between gap-3 p-4" : "dashboard-card-soft flex flex-wrap items-center justify-between gap-3 bg-accent-50 p-4"}>
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    ...
                    <p className="mt-1 text-sm text-dark-500">{item.type}</p>
                  </div>
                </div>
        ...
        <DashboardBlock title={isArabic ? "quick finance actions" : "Quick finance actions"} eyebrow={isArabic ? "next step" : "next step"}>
          <div className="grid gap-4">
            <SoftCard>
              <p className="text-xs uppercase tracking-[0.22em] text-dark-400">{isArabic ? "pending rail" : "Pending rail"}</p>
              <p className="mt-3 text-lg font-semibold text-dark-950">{formatCurrency(locale, data.pendingWithdrawal)}</p>
              <p className="mt-2 text-sm leading-7 text-dark-600">{isArabic ? "el amount da mostani el cycle el gaya lel payout." : "This amount is waiting for the next payout cycle."}</p>
            </SoftCard>
            <SoftCard>
              <p className="text-xs uppercase tracking-[0.22em] text-dark-400">{isArabic ? "ops links" : "Ops links"}</p>
              <div className="mt-4 flex flex-wrap gap-3">
                <Link href={`/${locale}/worker/requests/incoming`} className="rounded-full border border-dark-200 bg-white px-4 py-2 text-sm font-semibold text-dark-700 shadow-soft">
                  {isArabic ? "incoming" : "Incoming jobs"}
                </Link>
                <Link href={`/${locale}/worker/requests/active`} className="rounded-full border border-dark-200 bg-white px-4 py-2 text-sm font-semibold text-dark-700 shadow-soft">
