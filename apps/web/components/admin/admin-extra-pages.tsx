        ...
        {data.clients.length === 0 ? (
          ...
          <div className="grid gap-4 xl:grid-cols-2">
            {data.clients.map((client, index) => (
              <div key={client.id} className={index % 2 === 0 ? "dashboard-card-soft p-5" : "dashboard-card-soft bg-accent-50 p-5"}>
                <div className="flex items-start justify-between gap-3">
                  <div>
                    <h2 className="text-2xl font-semibold text-dark-950">{client.name}</h2>
                    <p className="mt-2 text-dark-500">{client.city}</p>
                  </div>
      ...
      <DashboardBlock title={isArabic ? "تدفق الطلبات" : "Request stream"} eyebrow={isArabic ? "بطاقات التشغيل" : "ops deck"}>
        <div className="grid gap-4">
          {data.requests.map((request, index) => (
            <div key={request.id} className={index % 2 === 0 ? "dashboard-card-soft p-5" : "dashboard-card-soft bg-surface-peach p-5"}>
              <div className="flex items-start justify-between gap-3">
                <div>
                  <h2 className="text-xl font-semibold text-dark-950">{request.title}</h2>
                  <p className="mt-2 text-dark-500">{request.city}</p>
                </div>
        ...
        <DashboardBlock title={isArabic ? "مصادر المال" : "Finance streams"} eyebrow={isArabic ? "طبقات التدفق" : "money layers"}>
          <div className="grid gap-4">
            {data.streams.map((item, index) => (
              <SoftCard key={item.label} className={index % 2 === 0 ? undefined : "bg-accent-50"}>
                <div className="flex items-center justify-between gap-3">
                  <p className="font-semibold text-dark-950">{item.label}</p>
                  <p className="text-lg font-semibold text-dark-950">{formatCurrency(locale, item.value)}</p>
                </div>
              </SoftCard>
            ))}
          </div>
        ...
        <DashboardBlock title={isArabic ? "مسار التحويل" : "Payout rail"} eyebrow={isArabic ? "بطاقات الإفراج" : "release deck"}>
          <div className="grid gap-4">
            {data.payouts.map((item, index) => (
              <div key={item.id} className={index % 2 === 0 ? "dashboard-card-soft p-4" : "dashboard-card-soft bg-surface-peach p-4"}>
                <div className="flex items-center justify-between gap-3">
                  <div>
                    <p className="font-semibold text-dark-950">{item.label}</p>
                    <p className="mt-1 text-sm text-dark-500">{item.status}</p>
                  </div>
                  <p className="text-lg font-semibold text-dark-950">{formatCurrency(locale, item.amount)}</p>
                </div>
              </div>
            ))}
          </div>
    ...
    <div>
      ...
      <div className="grid gap-6 xl:grid-cols-[1fr_1fr]">
        <DashboardBlock title={isArabic ? "المنصة" : "Platform"} eyebrow={isArabic ? "إعدادات الهوية" : "brand controls"}>
          <div className="grid gap-4">
            <label className="space-y-2"><span className="text-sm text-dark-500">{isArabic ? "بريد الدعم" : "Support email"}</span><input value={data.platform.supportEmail} onChange={(event) => setData({ ...data, platform: { ...data.platform, supportEmail: event.target.value } })} className="h-12 w-full rounded-[1rem] border border-dark-200 bg-white px-4" /></label>
            <label className="space-y-2"><span className="text-sm text-dark-500">{isArabic ? "خط الطوارئ" : "Emergency hotline"}</span><input value={data.platform.emergencyHotline} onChange={(event) => setData({ ...data, platform: { ...data.platform, emergencyHotline: event.target.value } })} className="h-12 w-full rounded-[1rem] border border-dark-200 bg-white px-4" /></label>
            <label className="space-y-2"><span className="text-sm text-dark-500">{isArabic ? "اللغة الافتراضية" : "Default language"}</span><input value={data.platform.defaultLanguage} onChange={(event) => setData({ ...data, platform: { ...data.platform, defaultLanguage: event.target.value } })} className="h-12 w-full rounded-[1rem] border border-dark-200 bg-white px-4" /></label>
          </div>
        </DashboardBlock>

        <DashboardBlock title={isArabic ? "التشغيل والإشراف" : "Operations + moderation"} eyebrow={isArabic ? "خيارات السياسات" : "policy toggles"}>
          <div className="grid gap-4">
            <label className="dashboard-card-soft flex items-center justify-between gap-4 p-4"><span className="font-medium text-dark-950">{isArabic ? "إسناد تلقائي" : "Auto assignment"}</span><input type="checkbox" checked={data.operations.autoAssignmentEnabled} onChange={(event) => setData({ ...data, operations: { ...data.operations, autoAssignmentEnabled: event.target.checked } })} className="h-4 w-4" /></label>
            <label className="dashboard-card-soft flex items-center justify-between gap-4 p-4"><span className="font-medium text-dark-950">{isArabic ? "توثيق يدوي إجباري" : "Manual verification required"}</span><input type="checkbox" checked={data.operations.manualVerificationRequired} onChange={(event) => setData({ ...data, operations: { ...data.operations, manualVerificationRequired: event.target.checked } })} className="h-4 w-4" /></label>
            <SplitInfo
              items={[
